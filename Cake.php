...
if(isset($_POST['filter']))
   ...
   {
    $result="SELECT * FROM `product`   WHERE `product`.`Name` LIKE '%Cake%' ORDER BY (`product`.`Price`);";
   ...
   elseif(isset($_POST['rangeFilter']))
   {
        $minVal=$_POST['mini'];
        $maxVal=$_POST['max'];
        $result="SELECT * FROM `product` WHERE `product`.`Name` LIKE '%Cake%' AND `product`.`Price` BETWEEN $minVal AND $maxVal;";
        $select=mysqli_query($conn,$result);
   }
    
   else
   {
    $result="SELECT * FROM `product`   WHERE `product`.`Name` LIKE '%Cake%' ;";
    $select=mysqli_query($conn,$result);
...
<?php while($row=mysqli_fetch_assoc($select)){ ?>
    ...
    <a href="Cake.php?Buy_Now=<?php echo $row['Id'];?>" class="btn1">
